...
public class ServerSodaLauncher {
	...
	public void launch(final Protocol protoc, int tcpPort,
			final ServerSodaListener l, Subscriptions subs) {
		launch(protoc, tcpPort, l, AuthService.NO_AUTH_SVC, subs);
	}

	public void launch(final Protocol protoc, int tcpPort,
			ServerSodaListener l, AuthService auth, Subscriptions subs) {
	...
	public void launch(final ServerConfig config, final ServerSodaListener l) {
		...
		SourceHonestyFilter filter = new SourceHonestyFilter(
				config.getProtocol());
		...
		config.configure(jettyServer, wampocServer, webSocketHandler);
		...
		try {
			...
			Thread t = new Thread(new Runnable() {
				...
				@Override
				public void run() {
					ServerSoda soda = new ServerSoda(config.getProtocol(),
							config.getAuthService(), config.getPath(),
							config.getPort());
